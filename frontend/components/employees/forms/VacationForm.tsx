 ...
 * Form for creating or editing vacation records (typ=urlop)
 */
...
export function VacationForm({
  ...
  onCancel,
  isLoading = false,
  submitLabel = initialData ? 'Zapisz zmiany' : 'Dodaj urlop',
  ...
  // Calculate vacation days dynamically
  const vacationDays =
  ...
  const handleFormSubmit = async (data: VacationFormData) => {
    try {
      ...
      console.error('Błąd przy zapisywaniu urlopu:', error);
    }
  };
  ...
  return (
    <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-4">
      ...
      {dataOd && dataDo && vacationDays > 0 && (
        <div className="rounded-md bg-blue-50 p-3">
          <p className="text-sm text-blue-900">
            <span className="font-medium">Liczba dni urlopu:</span>{' '}
            <span className="text-lg font-bold">{vacationDays}</span>
          </p>
        </div>
      )}

      {/* Hidden field for typ_nieobecnosci */}
      <input type="hidden" {...register('typ_nieobecnosci')} value="urlop" />
