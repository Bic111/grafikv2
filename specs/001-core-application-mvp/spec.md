...
### Session 2025-11-04
- Q: W jaki sposób powinien być zabezpieczony dostęp do aplikacji? → A: Brak zabezpieczeń. Aplikacja uruchamia się natychmiast, a każda osoba z dostępem do komputera może z niej korzystać.
- Q: Jakiego rodzaju mechanizm AI powinien zostać użyty? → A: Model hybrydowy. Proste przypadki rozwiązywane są lokalnie, a bardziej złożone wysyłane do zewnętrznej usługi (Gemini API).
- Q: Jaka jest przewidywana maksymalna liczba pracowników, którą system powinien być w stanie płynnie obsłużyć? → A: Zazwyczaj 15, maksymalnie 20 pracowników.
- Q: Jak system powinien obsługiwać niekrytyczne problemy lub ostrzeżenia? → A: Generator zapewnia poprawność grafiku; ostrzeżenia wyświetlane użytkownikowi do ręcznej korekty lub zatwierdzenia.
- Q: Co w przypadku, gdy liczba pracowników jest niewystarczająca do obsadzenia wszystkich zmian? → A: System wygeneruje grafik z brakami, wyraźnie je wskazując, i wyświetli ostrzeżenie.
